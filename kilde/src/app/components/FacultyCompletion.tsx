...
function kullVal(k: CompletionCohort, id: string): number | null {
  const n1ok = k.normertAar != null && k.normertAar + 1 <= NÅ - 1;
  const n2ok = k.normertAar != null && k.normertAar + 2 <= NÅ - 1;
  switch (id) {
    case 'normert': return pct(k.fullfortNormert, k.startkull);
    case 'normert1': return n1ok ? pct(k.fullfort1, k.startkull) : null;
    case 'normert2': return n2ok ? pct(k.fullfort2, k.startkull) : null;
    case 'frafalt': return pct(k.frafalt, k.startkull);
    case 'frafalt2': return n2ok ? pct(k.frafalt2, k.startkull) : null;
...
function value(p: CompletionProgram, m: MetricDef, year: number): number | null {
  if (m.kind === 'kull') { const k = p.kull.find((x) => x.aar === year); return k ? kullVal(k, m.id) : null; }
...
export function FacultyCompletion({ faculty }: { faculty: FacultyData }) {
  ...
  const years = useMemo(() => {
    ...
    group.programs.forEach((p) => (metric.kind === 'kull' ? p.kull : p.aar).forEach((x) => { if (value(p, metric, x.aar) != null) ys.add(x.aar); }));
    return [...ys].sort();
  }, [group, metric]);
  ...
  const chartData = years.map((y) => {
    ...
    entries.forEach((p) => { const v = value(p, metric, y); if (v != null) row[p.entryId] = Math.round(v * 10) / 10; });
    return row;
  });
  ...
  return (
    <div>
      ...
      {nmbu.map((p) => {
        ...
        const items = [
          { l: `Fullført normert · kull ${lastK?.aar ?? ''}`, v: lastK ? fmt(kullVal(lastK, 'normert'), METRICS[0]) : '–' },
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: 'minmax(240px, 280px) 1fr' }}>
        ...
        <div className="rounded-2xl p-5" style={{ backgroundColor: '#fff', border: '1px solid var(--nmbu-neutral-3)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--nmbu-neutral-2)', marginBottom: 8 }}>{metric.desc}{metric.kind === 'kull' ? ' · x-aksen er startår for kullet' : ''}</div>
            ...
            chartData.some((r) => Object.keys(r).length > 1) ? (
              <ResponsiveContainer width="100%" height={340}>
                <LineChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(v: number, name: string) => [fmt(v, metric), group.programs.find((p) => p.entryId === name)?.shortName ?? name]} labelFormatter={(l) => (metric.kind === 'kull' ? `Startkull ${l}` : String(l))} />
                  <Legend formatter={(v: string) => group.programs.find((p) => p.entryId === v)?.shortName ?? v} wrapperStyle={{ fontSize: 11 }} />
                  {entries.map((p) => (
          ...
          {tab === 'tabell' && (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {[...entries].sort((a, b) => (value(b, metric, tYear) ?? -1) - (value(a, metric, tYear) ?? -1)).map((p) => (
                    <tr key={p.entryId} style={{ borderBottom: '1px solid var(--nmbu-beige-light)', backgroundColor: p.isNmbu ? 'var(--nmbu-green-4)' : 'transparent' }}>
                      ...
                      {(metric.kind === 'kull' ? METRICS.filter((m) => m.kind === 'kull') : METRICS.filter((m) => m.kind === 'aar')).map((m) => (
                        <td key={m.id} className="px-3 py-2 text-right" style={{ fontWeight: m.id === metricId ? 700 : 400, whiteSpace: 'nowrap' }}>{fmt(value(p, m, tYear), m)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
              </table>
              <div style={{ fontSize: 11, color: 'var(--nmbu-neutral-2)', marginTop: 6 }}>{metric.kind === 'kull' ? `Startkull ${tYear}` : `År ${tYear}`}. «–» = ikke målt ennå eller ikke rapportert.</div>
            </div>
          )}
        </div>
      </div>
